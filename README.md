...
Open `http://SERVER:8000`. The first page requires creation of an administrator; there is no default password.

## 使用方法

### 直接使用 GitHub Container Registry 镜像

仓库和镜像为私有状态时，先使用具有 `read:packages` 权限的 GitHub Personal Access Token 登录：
...
打开 `http://服务器IP:8000` 创建首个管理员，然后进入 **Nodes → Add node** 添加 3x-ui 节点。Base URL 必须保留 3x-ui 的 WebBasePath，例如 `https://host:2053/abcdef`。
...
docker run -d --name trafficmanager -p 8000:8000 \
...
- Do not expose port 8000 directly to the Internet without HTTPS and appropriate network controls.
...
- **Container unhealthy:** run `docker logs trafficmanager` and `curl http://127.0.0.1:8000/health`; also verify `/data` is writable by the container.
