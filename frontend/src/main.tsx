...
const fmt = (n?: number | null) => n == null ? '—' : n === 0 ? '不限' : `${(n / 1073741824).toFixed(n > 10737418240 ? 0 : 1)} GiB`
  ...
  return <>
    ...
    <Table
      ...
      columns={[
        ...
        { title: '已用流量', dataIndex: 'used_bytes', render: fmt },
        { title: '配额', dataIndex: 'quota_bytes', render: fmt },
...
function Policies() {
  ...
  const create = async (values: any) => {
    try {
      await api('/api/policies', { method: 'POST', body: JSON.stringify({ ...values, quota_bytes: values.quota_gib == null ? null : Number(values.quota_gib) * 1073741824, local_time: values.local_time || '00:00', enabled: true, reset_enabled: values.reset_enabled ?? true, catchup_enabled: true, catchup_max_hours: 168, missing_day_policy: 'LAST_DAY', reactivate_mode: 'PRESERVE' }) })
